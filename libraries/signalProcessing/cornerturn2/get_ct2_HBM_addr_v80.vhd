...
-- 
-- Create Date: Jan 2026, modified from U55c version
-- Module Name: get_ct2_HBM_addr_v80 - Behavioral
-- Description: 
--  Calculate the address in HBM memory of a 256-byte block of data for corner turn 2.
--
--  The 256 byte blocks are arranged in a 3-D array (fine_channel, i_time_block, station)
--                                                        |                         |
--                                                  slowest changing          fastest changing
--
--  Where :
--   * i_time_block = 0 to 11, for 12 blocks of 16 time samples each (there are 16 time samples in a 256 byte block written to the HBM).
...
--    * 4 stations
--    * 2 polarisations
--    = 16 * 4 * 2 * (2 byte complex) = 256 bytes
--  
----------------------------------------------------------------------------------
...
entity get_ct2_HBM_addr_v80 is
    ...
    port(
        ...
        i_SB_HBM_base_Addr : in std_logic_vector(35 downto 0); -- Base address in HBM for this subarray-beam
        ...
        o_HBM_addr     : out std_logic_vector(35 downto 0); -- byte address in HBM, always 256-byte aligned (so low 8 bits will always be 0s).
...
architecture Behavioral of get_ct2_HBM_addr_v80 is
    ...
    signal demap_station_x64 : std_logic_vector(35 downto 0);
    --signal demap_station_x256_del1, demap_station_x256_del2 : std_logic_vector(31 downto 0);
    signal HBM_base_plus_station : std_logic_vector(35 downto 0);
    ...
    signal HBM_base_plus_station_del2, HBM_base_plus_station_del3 : std_logic_vector(35 downto 0);
    ...
    signal stations_x12_del3, stations_x12_del4, stations_x4_ext, stations_x8_ext : std_logic_vector(17 downto 0);
    
    signal coarse_diff_x_3456_p_fine_m_fstart_24bit : std_logic_vector(23 downto 0);
    signal fine_offset_del5 : signed(41 downto 0);
    signal HBM_base_station_time_del6, HBM_base_station_time_del5, HBM_base_station_time_del4, fine_offset_x256_del6 : std_logic_vector(35 downto 0);
...
begin
    ...
    demap_station_x64 <= "000000000000000000" & i_station(11 downto 2) & "00000000"; -- 32 bits = 13+10+9
    ...
    begin
        if rising_edge(i_axi_clk) then
            ...
            if (unsigned(i_time_block) > 11) then
                bad_time <= '1';
            else
                bad_time <= '0';
            end if;
            ...
            --   
            -- o_HBM_addr = i_SB_HBM_base_addr + 256 * [
            --               ((i_coarse_channel*3456 + i_fine_channel) - (i_SB_coarseStart * 3456 + i_SB_fineStart)) * 12 * ceil(i_SB_stations/4) + 
            --               i_time_block * ceil(i_SB_stations/4)
            --               floor(i_station/4)]
            --
            HBM_base_plus_station <= std_logic_vector(unsigned(i_SB_HBM_base_addr) + unsigned(demap_station_x64));
            ...
            if buffer_del1 = '0' then
                ...
                HBM_base_plus_station_del2 <= std_logic_vector(unsigned(HBM_base_plus_station) + unsigned(g_BUFFER_OFFSET));
            end if;
            ...
            ------------------------------------------------------------
            stations_x12_del3 <= std_logic_vector(unsigned(stations_x4_ext) + unsigned(stations_x8_ext));            
            ...
            time_x_stations_x256 <= "0000000" & std_logic_vector(time_x_stations_del3) & "00000000";
            stations_x12_del4 <= stations_x12_del3;
            ...
            -- (24 bit) * (18 bit)
            fine_offset_del5 <= signed(coarse_diff_x_3456_p_fine_m_fstart_24bit) * signed(stations_x12_del4);
            
            ------------------------------------------------------------
            HBM_base_station_time_del6 <= HBM_base_station_time_del5;
            fine_offset_x256_del6 <= std_logic_vector(fine_offset_del5(27 downto 0)) & "00000000";
            
            ------------------------------------------------------------
            HBM_base_station_time_fine_del7 <= std_logic_vector(unsigned(HBM_base_station_time_del6) + unsigned(fine_offset_x256_del6));
