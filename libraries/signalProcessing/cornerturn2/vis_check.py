...
def station_samples(station_info, sb, fine_rel_range, time_indices, integration):
    ...
    Returns a complex128 array of shape (2 pol, n_fine, n_time).
    pol 0 = Hpol, pol 1 = Vpol.
    ...
    for fi, fine_rel in enumerate(fine_rel_range):
        ...
        if fc < 0 or fc > 3455:
            ...
            continue
        hpol_re = _s8(fc & 0xFF)
        hpol_im = _s8(((fc >> 8) & 0x0F) | ((integration & 0x0F) << 4))
        out[0, fi, :] = complex(hpol_re, hpol_im)

    vpol_im = _s8(vc & 0xFF)
    for ti, t in enumerate(time_indices):
        vpol_re = _s8((t % 64) | (((t // 64) & 0x03) << 6))
        out[1, :, ti] = complex(vpol_re, vpol_im)
...
def compute_integration(sb, station_map, output_channel, output_time,
    ...
    for s in range(n_stations):
        if s in station_map:
            samp[s] = station_samples(station_map[s], sb, fine_rel_range,
                                       time_indices, integration)
            valid[s] = np.ones((len(fine_rel_range), n_time_group), dtype=np.int64)
        else:
...
def check_cell(dump, desc, sb, station_map, time_groups,
    ...
    """
    Check one cell.  Returns (n_vis_bad, n_meta_bad, n_missing).
    """
    vis, tci, fd = compute_integration(
        sb, station_map, desc['output_channel'], desc['output_time'],
        time_groups, desc['integration'])
            ...
            for p1 in range(2):
                for p2 in range(2):
                    ...
                    if act_re is None or act_im is None:
                        n_missing += 1
                        continue
                    if (abs(act_re - exp_re) > vis_atol + vis_rtol * abs(exp_re) or
                            abs(act_im - exp_im) > vis_atol + vis_rtol * abs(exp_im)):
                        n_vis_bad += 1
                        if detail_count[0] < max_detail:
                            detail_count[0] += 1
                            print(f"  VIS  cell={cell_index} e={e} "
    ...
    return n_vis_bad, n_meta_bad, n_missing
...
def check(cfg, dump, vis_rtol, vis_atol, tci_tol, max_detail, diagnose=False):
    ...
    for d in descs:
        if d['cell_index'] not in cell_set:
            continue
        vb, mb, ms = check_cell(dump, d, sb, station_map, time_groups,
    ...
    if (diagnose or n_vis_bad > 0 or n_meta_bad > 0) and cells:
        first_idx = cells[0]
    ...
    return n_checked, n_vis_bad, n_meta_bad, n_missing
...
def main():
    ...
    print("Checking ...\n")
    checked, vis_bad, meta_bad, missing = check(
    ...
    print(f"  Bad visibilities   : {vis_bad}")
    print(f"  Bad TCI/DV         : {meta_bad}")
    print(f"  Missing words      : {missing}")
    if checked == 0:
        print("  WARNING: nothing checked")
        sys.exit(2)
    elif vis_bad == 0 and meta_bad == 0 and missing == 0:
        print("  PASS")
        sys.exit(0)
    else:
        print("  FAIL")
