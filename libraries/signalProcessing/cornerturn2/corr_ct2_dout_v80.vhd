...
-- 
-- Create Date: jan 2026, based on previous u55c version
-- Module Name: corr_ct2_dout_v80 - Behavioral
-- Description: 
--  Readout to the correlator for the v80 version, which uses 256 byte data blocks in HBM instead of 512 byte blocks in the U55c.
-- 
-- Readout Pattern:
...
--                    |   |         Read HBM : 16 time samples, 16 stations                                                                     |     |
--                    |   |         Read HBM : Next 16 time samples, same 16 stations                                                           |     |
--                    |   |         Read HBM : Next 16 time samples, same 16 stations                                                           |     |
--                    |   |         Read HBM : Next 16 time samples, same 16 stations                                                           |     |
--                    |   |          - (Note each HBM read is 1024 bytes. A contiguous 1024 byte blocks in the HBM contains                     |     |
--                    |   |             data for 16 stations and 16 time samples)                                                               |     |
...
--  The address of 256-byte blocks of data is calculated by the "get_ct2_HBM_addr_v80" module, which is common to the data input side of the corner turn.
--
-- HBM data ordering :
--  Each 256 bit word in the HBM contains data for :
--     - 2 time samples, 4 consecutive stations
--  8x256 bits = 256 bytes. Each block of 256 bytes in the HBM contains data for :
--     - 16 time samples, 4 consecutive stations
--  4 x 256 bytes = 1024 bytes = size of HBM reads; Each block of 1024 bytes in the HBM contains data for :
--     - 16 time samples, 16 consecutive stations
--
----------------------------------------------------------------------------------
...
end corr_ct2_dout_v80;

architecture Behavioral of corr_ct2_dout_v80 is
    ...
    signal SB_base_addr : std_logic_vector(35 downto 0);        -- 36 bits,
...
begin
    ...
    port map(
        ...
        i_SB_HBM_base_Addr => SB_base_addr,   -- in (35:0); -- Base address in HBM for this subarray-beam
        ...
        i_time_block       => cur_timegroup(3 downto 0), -- in (3:0);  -- Which time block this is for; 0 to 11. Each time block is 16 time samples.
    ...
    -- Always read blocks of 256 bytes = 8 x (256 bit) words.
    -- There is data for 4 stations in each 256 byte block in HBM, so 1024 byte reads 
    -- returns data for 4x4=16 stations, i.e. the minimum amount used by the correlator array.
    o_HBM_axi_ar.len <= "00000111";
    o_HBM_axi_ar.addr(39 downto 36) <= "0000";
    o_HBM_axi_ar.addr(7 downto 0) <= "00000000";  -- All reads are 256 byte aligned.
                ...
                case ar_fsm is
                    ...
                    when wait_SB_data =>
                        ...
                        if i_SB_done = '1' then
                            ...
                            SB_base_addr <= "00" & i_HBM_base_addr & "00"; -- Convert to 36 bits, base address in HBM for this subarray-beam. The value in the table is a 4-byte word address.
                            SB_SB <= SB_del;
                            ...
                            cur_TimeGroup <= "0000"; -- steps through "0000", "0001", ..., "1011" for the 12 groups of 16 times, since data in HBM is written in 256 byte blocks with 16 time samples/block.
                    ...
                    when update_addr =>
                        ...
                        --                   |              Read HBM : Next 16 time samples, same 16 stations
                        --                   |              Read HBM : Next 16 time samples, same 16 stations
                        --    One full       |              Read HBM : Next 16 time samples, same 16 stations
                        ...
                        --    the correlator |                  Read HBM : 16 time samples, 16 stations
                        --    cell           |                  Read HBM : Next 16 time samples, same 16 stations 
                        --                   |                  Read HBM : Next 16 time samples, same 16 stations 
                        --                   |                  Read HBM : Next 16 time samples, same 16 stations 
                        --
                        ...
                        -- 
                        if cur_timegroup(1 downto 0) /= "11" then -- Get another block of 16 times
                            cur_timegroup(1 downto 0) <= std_logic_vector(unsigned(cur_timegroup(1 downto 0)) + 1); 
                            get_addr <= '1';
                            ar_fsm <= check_arFIFO;
                        else
                            cur_timegroup(1 downto 0) <= "00";
                    ...
                    when next_fine => -- Advance to the next fine channel within a group of fine channels that are being integrated.
                        ...
                        if (unsigned(cur_fineChannelOffset) = (unsigned(SB_fineIntegrations) - 1)) then
                            ...
                            if SB_timeIntegrations = '0' then
                                ...
                                case cur_timeGroup(3 downto 2) is -- bits 3:2 selects which of the three blocks of 283 ms we have just read out.
                                    when "00" => 
                                        cur_timeGroup <= "0100";
                                        get_addr <= '1';
                                        ar_fsm <= check_arFIFO;
    ...
    -- The dataFIFO has space for 512 words, i.e. 64 ar requests (each ar request is 256 bytes = 8 x 32 byte words)
    ...
    generic map (
        ...
        wr_rst_busy => open,       -- 1-bit output: Write Reset Busy: Active-High indicator that the FIFO write domain is currently in a reset state.
        din => i_HBM_axi_r.data(255 downto 0), -- WRITE_DATA_WIDTH-bit input: Write Data: The input data bus used when writing the FIFO.
        injectdbiterr => '0',      -- 1-bit input: Double Bit Error Injection
        injectsbiterr => '0',      -- 1-bit input: Single Bit Error Injection: 
