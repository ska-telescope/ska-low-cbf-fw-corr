...
entity corr_ct1_readout is
    ...
    Port(
        ...
        o_meta_delays : out t_CT1_META_delays_arr(3 downto 0); -- defined in DSP_top_pkg.vhd; fields are : HDeltaP(31:0), VDeltaP(31:0), HOffsetP(31:0), VOffsetP(31:0), bad_poly (std_logic)
        o_meta_RFIThresholds  : out t_slv_32_arr(3 downto 0);
...
begin
    ...
    o_delayTableAddr <= (others => '0');        
    o_RFI_rd_addr <= (others => '0'); 
    o_validMemReadAddr <= (others => '0'); 
    o_validMemWriteAddr <= (others => '0'); --: out std_logic_vector(18 downto 0); -- write always clears the memory (mark the block as invalid).
    o_validMemWrEn <= '0'; --     : out std_logic;
    
    o_sof <= '0'; --   : out std_logic; -- start of frame for a particular set of 4 virtual channels.
    o_sofFull <= '0'; --  : out std_logic; -- start of a full frame, i.e. 283 ms of data.
    o_readoutData <= (others => (others => '0')); -- : out t_slv_32_arr(3 downto 0);
    dgeni : for i in 0 to 3 generate
        o_meta_delays(i).HDeltaP <= (others => '0'); --: out t_CT1_META_delays_arr(3 downto 0); -- defined in DSP_top_pkg.vhd; fields are : HDeltaP(31:0), VDeltaP(31:0), HOffsetP(31:0), VOffsetP(31:0), bad_poly (std_logic)
        o_meta_delays(i).VDeltaP <= (others => '0');
        o_meta_delays(i).HOffsetP <= (others => '0');
        o_meta_delays(i).VOffsetP <= (others => '0');
        o_meta_delays(i).bad_poly <= '0';
    end generate;
    o_meta_RFIThresholds <= (others => (others => '0')); -- : out t_slv_32_arr(3 downto 0);
    o_meta_integration  <= (others => '0'); --   : out std_logic_vector(31 downto 0);
    o_meta_ctFrame       <= (others => '0'); --  : out std_logic_vector(1 downto 0); 
    o_meta_virtualChannel  <= (others => '0'); --: out std_logic_vector(11 downto 0); -- first virtual channel output, remaining 3 (U55c) or 11 (V80) are o_meta_VC+1, +2, etc.
    o_meta_valid  <= (others => '0'); --: out std_logic_vector(3 downto 0); -- Total number of virtual channels need not be a multiple of 12, so individual valid signals here.
    o_lastChannel <= '0'; --  : out std_logic; -- Aligns with o_metaX
    o_valid  <= '0'; -- : out std_logic;    
    
    o_axi_ar.valid <= '0'; --      : out t_axi4_full_addr; -- read address bus : out t_axi4_full_addr (.valid, .addr(39:0), .len(7:0))
    o_axi_ar.addr <= (others => '0');
    o_axi_ar.len <= (others => '0');
    o_axi_rready <= '0';
          
    o_readOverflow <= '0'; -- : out std_logic;     -- Pulses high in the shared_clk domain.
    o_Unexpected_rdata <= '0'; --: out std_logic; -- data was returned from the HBM that we didn't expect (i.e. no read request was put in for it)
    o_dataMissing <= '0'; --: out std_logic;      -- Read from a HBM address that we haven't written data to. Most reads are 8 beats = 8*64 = 512 bytes, so this will go high 16 times per missing LFAA packet.
    o_bad_readstart <= '0'; --: out std_logic;    -- read start occured prior to delay calculation finishing for the previous read.
    o_dFIFO_underflow  <= (others => '0'); --: out std_logic_vector(3 downto 0); -- Read of output fifos but they were empty
    -- Debug data
    o_dbg_vec <= (others => '0'); -- : out std_logic_vector(255 downto 0);
    o_dbg_valid <= '0'; -- : out std_logic;
    -- mismatch between output and expected when sending debug data inserted in lfaaIngest
    o_dbgCheckData0<= (others => '0'); -- : out std_logic_vector(31 downto 0);  -- expected
    o_dbgCheckData1 <= (others => '0'); --: out std_logic_vector(31 downto 0);
    o_dbgCheckData2<= (others => '0'); -- : out std_logic_vector(31 downto 0);
    o_dbgCheckData3 <= (others => '0'); --: out std_logic_vector(31 downto 0);
    o_dbgBadData0<= (others => '0'); -- : out std_logic_vector(31 downto 0);   -- actual first time it doesnt match expected
    o_dbgBadData1 <= (others => '0'); --: out std_logic_vector(31 downto 0);
    o_dbgBadData2 <= (others => '0'); --: out std_logic_vector(31 downto 0);
    o_dbgBadData3<= (others => '0'); -- : out std_logic_vector(31 downto 0);
    o_mismatch_set <= (others => '0'); --: out std_logic_vector(3 downto 0);
