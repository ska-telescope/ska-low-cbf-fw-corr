...
library IEEE, UNISIM, correlator_lib, ct_lib, common_lib, filterbanks_lib;
...
architecture Behavioral of ct1_v80_tb is
    ...
    constant M01_DATA_WIDTH : integer := 256;
    ...
    signal m01_awaddr : std_logic_vector(33 downto 0);
    ...
    signal m01_araddr :  std_logic_vector(33 downto 0);
    ...
    signal m02_araddr :  std_logic_vector(33 downto 0);
    ...
    signal m02_rdata :  std_logic_vector((M01_DATA_WIDTH-1) downto 0);
    ...
    signal wdata_fsm, wdata2_fsm : wdata_fsm_type := idle;
    signal wdata_virtual_channel, wdata2_virtual_channel : std_logic_vector(15 downto 0);
    signal wdata_packet_count, wdata2_packet_count : std_logic_vector(47 downto 0);
    signal absolute_sample, absolute_sample2 : std_logic_vector(95 downto 0);
    signal wdata_word_count, wdata2_word_count : std_logic_vector(47 downto 0);
    ...
    signal hbm_dump_filename, init_fname, init_fname2 : string(1 to 9) := "dummy.txt";
    signal hbm_dump_filename2 : string(1 to 10) := "dummy2.txt";
    ...
    constant hex_12 : std_logic_vector(3 downto 0) := "1100";
    ...
    signal fb_lastChannel, fb_lastChannel_del1, fb_lastChannel_del2, fb_lastChannel_del3, fb_lastChannel_del4, fb_demap_table_select : std_logic;
    signal fb2_demap_table_select : std_logic;
    signal fb3_demap_table_select : std_logic;
    signal fb4_demap_table_select : std_logic;
    ...
    signal fb_meta_valid, fb_meta_valid_del1, fb_meta_valid_del2, fb_meta_valid_del3, fb_meta_valid_del4 : std_logic_vector(11 downto 0);
    
    signal clk600, clk600_gated : std_logic := '0';
    signal clock_600_no_buffer : std_logic := '0';
...
begin
    ...
    clock_600_no_buffer <= not clock_600_no_buffer after 0.8333 ns;
    ...
    generic map (
        ...
        O1 => clk600,          -- 1-bit output: Buffer
        O2 => clk300,          -- 1-bit output: Buffer
    ...
    process
        ...
        variable regData, regAddr_base : std_logic_vector(31 downto 0);
        variable regAddr_4byte_base, regAddr : std_logic_vector(17 downto 0);
    ...
    begin
        ...
        while (not endfile(RegCmdfile)) loop
            ...
            -- get the address
            hread(regLine_in,regAddr_base,regGood);
            regAddr_4byte_base := regAddr_base(19 downto 2);
        ...
        noc_wr_adr <= std_logic_vector(to_unsigned(c_config_ripple_select_address.base_address + c_config_ripple_select_address.address,18));
        noc_wr_dat <= g_RIPPLE_SELECT;
        noc_wren <= '1';
        wait until rising_edge(clk300);
        wait for 1 ps;
        noc_wren <= '0';
        ...
        noc_wr_adr <= std_logic_vector(to_unsigned(c_config_table_select_address.base_address + c_config_table_select_address.address,18));
        noc_wr_dat <= x"00000003";
        noc_wren <= '1';
        wait until rising_edge(clk300);
        wait;
    end process;
    ...
    --
    -- Each SPS packet generates two HBM axi transactions, one on the "m01" and one on the "m02" interface
    -- Each packet is 4096 bytes, consisting of the first 32 bytes in each 64 byte block in the 8192 bytes in the full SPS packet
    process(clk300)
    begin
        if rising_Edge(clk300) then
                ...
                case wdata_fsm is
                    ...
                    when send_burst =>
                        if m01_wready = '1' then
                            if (unsigned(wdata_word_count) = 127) then
            ...
            if data_rst = '1' then
                wdata2_fsm <= idle;
                wdata2_virtual_channel <= (others => '0');
                wdata2_packet_count <= g_PACKET_COUNT_START;
                -- 8192 bytes per packet,
                -- split into 2 x 4096 bytes
                --  = 2 x 64 (64-byte) words
                wdata2_word_count <= (others => '0');
            else
                case wdata2_fsm is
                    when idle =>
                        wdata2_fsm <= send_burst;
                    
                    when send_burst =>
                        if m02_wready = '1' then
                            if (unsigned(wdata2_word_count) = 127) then
                                wdata2_fsm <= update_burst;
                            else
                                wdata2_word_count <= std_logic_vector(unsigned(wdata2_word_count) + 1);
                            end if;
                        end if;
                        
                    when update_burst =>
                        wdata2_word_count <= (others => '0');
                        if (unsigned(wdata2_virtual_channel) >= (g_VIRTUAL_CHANNELS-1)) then
                            wdata2_virtual_channel <= (others => '0');
                            wdata2_packet_count <= std_logic_vector(unsigned(wdata2_packet_count) + 1);
                        else
                            wdata2_virtual_channel <= std_logic_vector(unsigned(wdata2_virtual_channel) + 1);
                        end if;
                        wdata2_fsm <= send_burst;
    ...
    absolute_sample <= std_logic_vector(unsigned(wdata_packet_count) * 2048 + unsigned(wdata_word_count) * 16); --to_unsigned(16,48));
    ...
    dgen1 : for i in 0 to 7 generate
...
--    m01_axi_w.data(511 downto 128) <= (others => '0'); 
    ...
    m01_axi_w.last <= '1' when unsigned(wdata_word_count) = 127 else '0';


    ------------------------------------------------------------------------------------------------------------
    -- Second HBM interface
    -- 
    -- wdata holds the absolute sample index, relative to the epoch
    -- as a 24 bit value, plus an 8 bit virtual channel.
    -- 2048 samples per packet, 1024 samples per burst (of 4096 bytes), 16 samples per word (of 64 bytes)
    absolute_sample2 <= std_logic_vector(unsigned(wdata2_packet_count) * 2048 + unsigned(wdata2_word_count) * 16); --to_unsigned(16,48));
    -- if enabled, flag every sample as RFI in a block of 16*4096 = 65536 samples, then have an equal number of unflagged samples
    flag_rfi2 <= '1' when absolute_sample2(16) = '1' else '0';
    
    dgen2 : for i in 0 to 7 generate
        m02_axi_w.data(i*32+23 downto i*32+0) <= 
            x"808080"  when g_DATA_RFI = '1' and flag_rfi2 = '1' else
            x"000000"  when g_DATA_RFI = '1' and flag_rfi2 = '0' else
            absolute_sample2(23 downto 4) & std_logic_vector(to_unsigned(i+8,4));  -- +8 to get the second 32 bytes in every 64-byte block in the SPS packet 
        m02_axi_w.data(i*32+31 downto i*32+24) <= 
            x"80"  when g_DATA_RFI = '1' and flag_rfi2 = '1' else
            x"00"  when g_DATA_RFI = '1' and flag_rfi2 = '0' else
            wdata2_virtual_channel(7 downto 0);
        
    end generate;
    
    m02_axi_w.valid <= '1' when wdata2_fsm = send_burst else '0';
    m02_axi_w.last <= '1' when unsigned(wdata2_word_count) = 127 else '0';
    -----------------------------------------------------------------------------------------------------------
            ...
            if fb_valid = '1' then
                ...
                if fb_valid_del = '0' and fb_valid = '1' then
                    ...
                    for i in 0 to 11 loop
                        line_out := "";
                        hwrite(line_out,std_logic_vector(to_unsigned(i,4)),RIGHT,1);
                        hwrite(line_out,fb_meta_delays(i).HDeltaP,RIGHT,9);
                        hwrite(line_out,fb_meta_delays(i).HOffsetP,RIGHT,9);
                        hwrite(line_out,fb_meta_delays(i).VDeltaP,RIGHT,9);
                        hwrite(line_out,fb_meta_delays(i).VOffsetP,RIGHT,9);
                        hwrite(line_out,fb_meta_integration,RIGHT,9);
                        hwrite(line_out,"00" & fb_meta_ctFrame,RIGHT,2);
                        hwrite(line_out,std_logic_vector(unsigned(fb_meta_virtualChannel) + i),RIGHT,5);
                ...
                line_out := "";
                hwrite(line_out,hex_12,RIGHT,1);
                for i in 0 to 23 loop
                    hwrite(line_out,fb_data(i)(15 downto 0),RIGHT,5);
                    hwrite(line_out,fb_data(i)(31 downto 16),RIGHT,5);
                end loop;
    ...
    generic map (
        g_V80_SIMULATION => True
    ) port map (
        -- shared memory interface clock (300 MHz)
        i_shared_clk     => clk300, -- in std_logic;
        i_shared_clkx2   => clk600, -- in std_logic;
        ...
        o_m02_axi_aw => m02_axi_aw,       -- out t_axi4_full_addr; -- write address bus : out t_axi4_full_addr (.valid, .addr(39:0), .len(7:0))
        i_m02_axi_awready => m02_awready, -- in std_logic;
        -- b bus - write response
        i_m02_axi_b  => m02_axi_b,        -- in t_axi4_full_b;   -- (.valid, .resp); resp of "00" or "01" means ok, "10" or "11" means the write failed.
        -- ar bus - read address
        o_m02_axi_ar      => m02_axi_ar,  -- out t_axi4_full_addr; -- read address bus : out t_axi4_full_addr (.valid, .addr(39:0), .len(7:0))
        i_m02_axi_arready => m02_arready, -- in std_logic;
        -- r bus - read data
        i_m02_axi_r       => m02_axi_r,   -- in  t_axi4_full_data;
        o_m02_axi_rready  => m02_rready,  -- out std_logic;
        ...
        i_noc_wren_tb => noc_wren, -- in std_logic;
        i_noc_rden_tb => noc_rden, -- in std_logic;
        i_noc_wr_adr_tb => noc_wr_adr, -- in std_logic_vector(17 downto 0);
        i_noc_wr_dat_tb => noc_wr_dat, -- in std_logic_vector(31 downto 0);
        i_noc_rd_adr_tb => noc_rd_adr, -- in std_logic_vector(17 downto 0);
        o_noc_rd_dat_tb => noc_rd_dat  -- out std_logic_vector(31 downto 0)
    );
    
    m01_awaddr <= m01_axi_aw.addr(33 downto 0);
    m01_awlen <= m01_axi_aw.len;
    m01_awvalid <= m01_axi_aw.valid;
    
    m01_araddr <= m01_axi_ar.addr(33 downto 0);
    ...
    m02_awaddr <= m02_axi_aw.addr(33 downto 0);
    m02_awlen <= m02_axi_aw.len;
    m02_awvalid <= m02_axi_aw.valid;
    
    m02_araddr <= m02_axi_ar.addr(33 downto 0);
    m02_arlen <= m02_axi_ar.len;
    m02_arvalid <= m02_axi_ar.valid;
    
    -- Other unused axi ports
    m02_awsize <= get_axi_size(M01_DATA_WIDTH);
    ...
    m02_arsize  <= get_axi_size(M01_DATA_WIDTH);   -- 6 = 64 bytes per beat = 512 bit wide bus. -- out std_logic_vector(2 downto 0);
    ...
    generic map (
        ...
        LATENCY_ZERO_PROBABILITY => 98 -- natural := 80   -- probability, as a percentage, that the gap between read beats will be zero.
    ) port map (
        i_clk        => clk300,
        i_rst_n      => rst_n,
        axi_awaddr   => m01_awaddr(33 downto 0),
        ...
        axi_wdata    => m01_axi_w.data(255 downto 0),
        axi_wstrb    => m01_wstrb(31 downto 0),
        ...
        axi_araddr   => m01_araddr(33 downto 0),
        axi_arlen    => m01_arlen,
        ...
        axi_rdata    => m01_axi_r.data(255 downto 0),
    ...
    generic map (
        ...
        i_clk        => clk300,
        i_rst_n      => rst_n,
        axi_awaddr   => m02_awaddr(33 downto 0),
        ...
        axi_wdata    => m02_axi_w.data(255 downto 0),
        axi_wstrb    => m02_wstrb(31 downto 0),
        axi_wlast    => m02_axi_w.last,
        axi_wvalid   => m02_axi_w.valid,
        ...
        axi_araddr   => m02_araddr(33 downto 0),
        axi_arlen    => m02_arlen,
        ...
        axi_rdata    => m02_axi_r.data(255 downto 0),
        axi_rresp    => m02_axi_r.resp,
        axi_rlast    => m02_axi_r.last,
        axi_rvalid   => m02_axi_r.valid,
        axi_rready   => m02_rready,
        i_write_to_disk => write_HBM_to_disk, -- in std_logic;
        i_write_to_disk_addr => hbm_dump_addr, -- address to start the memory dump at.
        -- 4 Mbytes = first 8 data streams
        i_write_to_disk_size => 4194304, -- size in bytes
        i_fname => hbm_dump_filename2, -- : in string
        -- Initialisation of the memory
        -- The memory is loaded with the contents of the file i_init_fname in 
        -- any clock cycle where i_init_mem is high.
        i_init_mem   => init_mem2, --  in std_logic;
        i_init_fname => init_fname2 -- : in string
    );
    ...
    clk600_gated <= clk600 when clk_gate = '1' else '0';
            ...
            loop
                ...
                if FD_dataValid = '1' then
                    ...
                    if FD_dataValid_del = '0' and FD_dataValid = '1' then
                        ...
                        hwrite(line_out,FD_virtualChannel(4),RIGHT,5);
                        hwrite(line_out,FD_virtualChannel(5),RIGHT,5);
                        hwrite(line_out,FD_virtualChannel(6),RIGHT,5);
                        hwrite(line_out,FD_virtualChannel(7),RIGHT,5);
                        hwrite(line_out,FD_virtualChannel(8),RIGHT,5);
                    ...
                    hwrite(line_out,fd_data(4).Hpol.re,RIGHT,3);
                    hwrite(line_out,fd_data(4).Hpol.im,RIGHT,3);
                    hwrite(line_out,fd_data(4).Vpol.re,RIGHT,3);
                    hwrite(line_out,fd_data(4).Vpol.im,RIGHT,3);
                    hwrite(line_out,fd_data(5).Hpol.re,RIGHT,3);
                    hwrite(line_out,fd_data(5).Hpol.im,RIGHT,3);
                    hwrite(line_out,fd_data(5).Vpol.re,RIGHT,3);
                    hwrite(line_out,fd_data(5).Vpol.im,RIGHT,3);
                    hwrite(line_out,fd_data(6).Hpol.re,RIGHT,3);
                    hwrite(line_out,fd_data(6).Hpol.im,RIGHT,3);
                    hwrite(line_out,fd_data(6).Vpol.re,RIGHT,3);
                    hwrite(line_out,fd_data(6).Vpol.im,RIGHT,3);
                    hwrite(line_out,fd_data(7).Hpol.re,RIGHT,3);
                    hwrite(line_out,fd_data(7).Hpol.im,RIGHT,3);
                    hwrite(line_out,fd_data(7).Vpol.re,RIGHT,3);
                    hwrite(line_out,fd_data(7).Vpol.im,RIGHT,3);
                    
                    hwrite(line_out,fd_data(8).Hpol.re,RIGHT,3);
                    hwrite(line_out,fd_data(8).Hpol.im,RIGHT,3);
                    hwrite(line_out,fd_data(8).Vpol.re,RIGHT,3);
                    hwrite(line_out,fd_data(8).Vpol.im,RIGHT,3);
                    hwrite(line_out,fd_data(9).Hpol.re,RIGHT,3);
                    hwrite(line_out,fd_data(9).Hpol.im,RIGHT,3);
                    hwrite(line_out,fd_data(9).Vpol.re,RIGHT,3);
                    hwrite(line_out,fd_data(9).Vpol.im,RIGHT,3);
                    hwrite(line_out,fd_data(10).Hpol.re,RIGHT,3);
                    hwrite(line_out,fd_data(10).Hpol.im,RIGHT,3);
                    hwrite(line_out,fd_data(10).Vpol.re,RIGHT,3);
                    hwrite(line_out,fd_data(10).Vpol.im,RIGHT,3);
                    hwrite(line_out,fd_data(11).Hpol.re,RIGHT,3);
                    hwrite(line_out,fd_data(11).Hpol.im,RIGHT,3);
                    hwrite(line_out,fd_data(11).Vpol.re,RIGHT,3);
                    hwrite(line_out,fd_data(11).Vpol.im,RIGHT,3);                                        
