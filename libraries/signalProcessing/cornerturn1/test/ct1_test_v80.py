...
def get_tb_data(tb_file, virtual_channels):
    ...
    # File Format : text
    #   - 12 lines of meta data, one per channel
    ...
    vc_list = np.zeros(12,dtype = np.int32)
    for line in tb_file:
        dval = line.split()
        dint = [int(di,16) for di in dval]
        if dint[0] == 0:
        ...
        if (dint[0] == 0 or dint[0] == 1 or dint[0] == 2 or dint[0] == 3 or 
            dint[0] == 4 or dint[0] == 5 or dint[0] == 6 or dint[0] == 7 or
            dint[0] == 8 or dint[0] == 9 or dint[0] == 10 or dint[0] == 11):
            ...
            for vc_list_ind in range(12):
                data_data[integration, frame, packet_count[integration,frame,vc] - 1, vc_list[vc_list_ind], 0, dcount] = conv_signed_16bit(dint[vc_list_ind*4 + 1])
                data_data[integration, frame, packet_count[integration,frame,vc] - 1, vc_list[vc_list_ind], 1, dcount] = conv_signed_16bit(dint[vc_list_ind*4 + 2])
                data_data[integration, frame, packet_count[integration,frame,vc] - 1, vc_list[vc_list_ind], 2, dcount] = conv_signed_16bit(dint[vc_list_ind*4 + 3])
                data_data[integration, frame, packet_count[integration,frame,vc] - 1, vc_list[vc_list_ind], 3, dcount] = conv_signed_16bit(dint[vc_list_ind*4 + 4])
            dcount = dcount + 1
...
def main():
    ...
    # RFI thresholds are (up to) 3072 words, starting at byte address 491520 = x78000 (or 4-byte word address 0x1E000)
    RFI_addr = 491520
